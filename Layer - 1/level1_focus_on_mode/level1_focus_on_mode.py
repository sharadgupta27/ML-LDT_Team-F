...
class Level1_Focus_on_mode:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'Level1_Focus_on_mode_{}.qm'.format(locale))
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('Level1_Focus_on_mode', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/level1_focus_on_mode/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'Focus-on Mode'),
            callback=self.run,
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(
                self.tr(u'&Level 1 Focus-on mode'),
                action)
    ...
    def read_start_end(self):
        start = self.dlg.dateEdit_2.date().toPyDate()
        start = str(start)
        start = '/'.join(start.split('-'))
        end = self.dlg.dateEdit_3.date().toPyDate()
        end = str(end)
        end = '/'.join(end.split('-'))
        return [start, end]
    ...
    def final(self):
        output_directory = str(self.dlg.lineEdit_4.text())
        clipper = self.clipper_select()
        if clipper[-1] == 'shape':
            shape_file = self.clipper_select()[0]
        else:
            pass
        self.dlg.close()
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg = Level1_Focus_on_modeDialog()
